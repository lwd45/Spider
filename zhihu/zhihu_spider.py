...
def save_content(title, author_name, author_url, image_urls, request_header):
    save_path = title + '/' + author_name
    if not os.path.exists(save_path):  # 创建保存路径文件
        os.makedirs(save_path)

    # 保存用户主页url
    with open(save_path + '/author_url.txt', 'w', encoding='gb18030', errors='ignore') as f:
        f.writelines(author_url)

    # 开始下载用户图片
    percentage, total = 1, len(image_urls)
    for img_url in image_urls:
        filename = save_path + '/' + str(random.randint(1000000, 9999999)) + ".jpg"
